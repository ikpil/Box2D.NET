...
public class ReplayFile : Sample
{
    ...
    // Advance one recorded step, keep the world pointer current, and publish the timing data
    // generated by the b2World_Step dispatched from the recording.
    private bool AdvanceOne()
    {
        bool stepped = b2RecPlayer_StepFrame(m_player);
        m_worldId = b2RecPlayer_GetWorldId(m_player);
        if (stepped)
        {
            CaptureProfile();
    ...
    // Seeking may restart and fast-forward the player internally. Start a fresh graph so timings
    // from unrelated timeline ranges are not averaged together, then retain the target frame's
    // final world profile when the target contains at least one step.
    private void SeekFrame(int targetFrame)
    {
        int previousFrame = b2RecPlayer_GetFrame(m_player);
        b2RecPlayer_SeekFrame(m_player, targetFrame);
        m_worldId = b2RecPlayer_GetWorldId(m_player);
        m_frameAccum = 0.0f;

        int currentFrame = b2RecPlayer_GetFrame(m_player);
