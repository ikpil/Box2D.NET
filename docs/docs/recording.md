...
Box2D.NET can record a simulation to a file and replay it later, reproducing the original run
...
- **Pointer width** and **endianness** are enforced. The format stores definitions field by field,
  and `b2RecPlayer_Create` and `b2ValidateReplayFile` reject a file with an incompatible header
  rather than producing a silently wrong replay. The format version is enforced the same way.
- **Floating-point environment** must match. Box2D.NET follows the upstream operation order and
  does not deliberately introduce fused multiply-add operations into the simulation paths.
...
```csharp
uint engineHash = b2GetBuildHash();                    // the running engine
uint fileHash = b2RecPlayer_GetBuildHash(player);      // the recording
```

The hash is stamped at build time. This only makes the warning coarse and never blocks replay.
The Replay sample shows the recorded hash on load and draws a `build mismatch` line when the two
differ and neither is zero.
...
- **User data is not preserved.** `userData` references are host objects with no meaning in
  another replay process, so they are written as zero. Code that keys off user data during replay
  will not see the original objects.
